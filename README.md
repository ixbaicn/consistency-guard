...
```text
用 consistency-guard 检查当前改动。低风险不确定项请先做临时假设继续审查，最后集中列出需要我决定的问题；只有权限、支付、数据迁移、破坏性修改这类高风险项才停下来问我。
```
...
让它“低风险先假设，最后集中问”。这个 skill 应该只在高风险修改、业务语义缺失、权威来源冲突、无法安全执行时停下来；普通文档、测试、命名、低风险审查问题应继续推进并记录假设。
